...
class SessionAPIView(APIView):
    def post(self, request, *args, **kwargs):
        ...
        if not token:
            return Response({'message': '올바른 토큰이 아닙니다.'}, status=status.HTTP_401_UNAUTHORIZED)

        if auth_number != token.auth_number:
            return Response({'message': '인증번호가 틀립니다.'}, status=status.HTTP_401_UNAUTHORIZED)

        is_expired = token.expired_at <= now()
        if is_expired:
            return Response({'message': "인증시간이 만료 되었습니다."}, status=status.HTTP_401_UNAUTHORIZED)
